...
self.addEventListener("install", function(event) {
  ...
  event.waitUntil(
    caches
      .open(CACHE_STATIC) // store the asstes to cache
      .then(function(cache) {
...
self.addEventListener("activate", function(event) {
  ...
  event.waitUntil(
    caches
      .keys() // to store the list of cache in the storage
      .then(function(keyList) {
        return Promise.all(
          keyList.map(function(key) {
            // convert list array to promise wait untul all list is get
            if (key !== CACHE_STATIC && key !== CACHE_DYNAMIC) {
              // check if the name of list cache not the current our assets
